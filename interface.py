...
def dividir_binario(texto, fuente, max_ancho):
    chars_por_linea = max_ancho // fuente.size("1")[0]
...
def procesar_texto(texto):
    ...
    binario_codificado = Arbol.text_to_bin(texto)
...
def guardar_bin():
    ...
    if not binario_codificado:
        ...
        return
    ruta_destino = filedialog.asksaveasfilename(defaultextension=".bin", filetypes=[("Archivos binarios", "*.bin")])
    if not ruta_destino:
        return
    try:
...
def dibujar_encriptar():
    ...
    mouse = pygame.mouse.get_pos()
    pygame.draw.rect(VENTANA, HOVER if boton_rect.collidepoint(mouse) else AZUL, boton_rect)
    pygame.draw.rect(VENTANA, HOVER if boton_guardar_bin.collidepoint(mouse) else AZUL, boton_guardar_bin)
...
def dibujar_desencriptar():
    ...
    dibujar_caja_output(texto_output)
    mouse = pygame.mouse.get_pos()
    pygame.draw.rect(VENTANA, HOVER if boton_cargar_bin.collidepoint(mouse) else AZUL, boton_cargar_bin)
    pygame.draw.rect(VENTANA, HOVER if boton_desencriptar.collidepoint(mouse) else AZUL, boton_desencriptar)
...
# Eventos
def manejar_eventos():
    global texto_input, texto_output, activo, pantalla_actual, scroll_output, scroll_binario
    for evento in pygame.event.get():
        if evento.type == pygame.QUIT:
            ...
            if pantalla_actual == "encriptar":
                scroll_output = max(0, scroll_output - evento.y)
            elif pantalla_actual == "desencriptar":
                scroll_binario = max(0, scroll_binario - evento.y)
